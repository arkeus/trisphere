class User < ActiveRecord::Base
	...
	def befriend(user, note = nil)
		...
		raise "Cannot add friend, '#{user.username}' is already a friend!" if friendships.find { |u| u.friend_id == user.id }
		friendships << Friendship.new(user_id: self.id, friend_id: user.id, note: note)
	end
